...
def getlog(conn, address, thread_name):
    ...
    channel.get_pty()
    # execute command
    command = 'tail -f /usr/local/apache-tomcat-8.5.20/logs/catalina.out.2018-10-18.out'
    ...
    while True:
        try:
            ...
            while channel.recv_ready():
                recvfromssh = channel.recv(1637)
                # print recvfromssh
                # print type(recvfromssh)
                # log = re.findall("\[(.*?)\]\[(.*?)\],({.*})", recvfromssh)
                log = recvfromssh.split("\n")
                print type(log)
                print len(log)
